...
public:
    ...
    [[= splice::hookable{}]] void withdraw(float amount) {
        balance -= amount;
        std::println("  withdrew {:.2f}, balance is now {:.2f}", amount, balance);
    }

    [[= splice::hookable{}]] void transfer(std::string_view to, float amount) {
        balance -= amount;
        std::println("  transferred {:.2f} to {}", amount, to);
    }
};

SPLICE_REGISTRY(BankAccount, g_account);

void setupValidation() {
    // Reject withdrawals of zero or negative amounts.
    g_account->inject<^^BankAccount::withdraw, splice::InjectPoint::Head>(
        [](splice::CallbackInfo& ci, BankAccount*, float amount) {
            if (amount <= 0.0f) {
                std::println("[validation] rejected withdrawal — amount must be positive (got {:.2f})",
                    amount);
                ci.cancelled = true;
            }
        },
        splice::Priority::Highest);

    // Reject withdrawals that would overdraw the account.
    g_account->inject<^^BankAccount::withdraw, splice::InjectPoint::Head>(
        [](splice::CallbackInfo& ci, BankAccount* self, float amount) {
    ...
    g_account->inject<^^BankAccount::transfer, splice::InjectPoint::Head>(
        [](splice::CallbackInfo& ci,
           BankAccount*,
           std::string_view to,
           float) {
            if (to.empty()) {
