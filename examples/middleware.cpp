...
public:
    [[= splice::hookable{}]] float processSample(float sample) {
        return sample;
    }
};

SPLICE_REGISTRY(AudioProcessor, g_audio);
...
int main() {
    ...
    float out1 = g_audio->dispatch<^^AudioProcessor::processSample>(
        &processor, 0.5f);
    std::println("  final: {:.4f}\n", out1);

    std::println("--- sample out of range ---");
    float out2 = g_audio->dispatch<^^AudioProcessor::processSample>(
        &processor, 2.5f);
